...
[[ -f "$PI_AI_DIST/providers/faux.js" ]] || { echo "error: pi-ai faux provider not found at $PI_AI_DIST — is pi installed?" >&2; exit 1; }
...
for turns in "${TURN_COUNTS[@]}"; do
  ...
  rho_rss="$(max_rss_bytes "$WORK/rho.time")"; tau_rss="$(max_rss_bytes "$WORK/tau.time")"; pi_rss="$(max_rss_bytes "$WORK/pi.time")"
  if [[ -z "$rho_rss" || -z "$tau_rss" || -z "$pi_rss" ]]; then
    echo "error: could not parse peak RSS from /usr/bin/time output" >&2
    cat "$WORK/rho.time" "$WORK/pi.time" >&2; exit 1
  fi
  rho_msg="$(cat "$WORK/rho.out")"; tau_msg="$(cat "$WORK/tau.out")"; pi_msg="$(cat "$WORK/pi.out")"
  echo "   rho: $rho_msg  peak=$rho_rss B    tau: $tau_msg  peak=$tau_rss B    pi: $pi_msg  peak=$pi_rss B"
