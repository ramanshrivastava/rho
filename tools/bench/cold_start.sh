...
mkdir -p "$RHO_HOME" "$TAU_HOME" "$PI_HOME"

# pi (TypeScript/Node, the original agent both tau and rho port) is measured two
# ways, mirroring tau's uv-vs-venv treatment:
#   * "pi" via the fnm PATH shim — what a user actually types (adds fnm's
...
# pi -p reads stdin when piped, so redirect </dev/null or it blocks. PI_OFFLINE
# disables the startup version/catalog network probe (the fair, isolated spawn).
PI_PRINT="env PI_CODING_AGENT_DIR=$PI_HOME OPENAI_API_KEY=dummy PI_OFFLINE=1 $PI_SHIM -p 'Say hello' --provider bench --model $MODEL --mode text --no-session </dev/null"
...
# usual entry here), so this row includes uv's launcher. pi gets two rows — the
