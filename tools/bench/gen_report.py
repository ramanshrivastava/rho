...
def render_md(records: list[dict], meta: dict) -> str:
    ...
    if sr_rho:
        ...
        a("**Parse dominates on all sides** (replay of a linear log is trivially "
          ...
          "byte-compat — which is far from free. So over tau, rho posts a solid "
          "**several-fold** win, not the ~100× of the allocation-light micro-benches: "
          "this is where rho's compatibility constraints cost it the most. **pi is the "
          "surprise: on the `linear` rows it is the fastest of the three** — V8's "
          "JIT-compiled `JSON.parse` plus a light id/parentId tree-walk beats both "
          "pydantic and rho's trial-decoding untagged union, so rho's win over tau does "
          "*not* carry to pi. Two honest caveats scope the pi column: (1) **same "
